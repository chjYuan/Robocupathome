...
jointlift::~jointlift()
{
 ...
 void jointlift::action(double (&pose)[8])
    ...
    void jointlift::movejoint_1(double (&pose)[6])
    {
     geometry_msgs::PoseStamped goal_pose;
    ...
    goal_pose.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(pose[3], pose[4], pose[5]);
     ...
     //choose your preferred planner
     group_arm_torso.setPlannerId("SBLkConfigDefault");
     group_arm_torso.setPoseReferenceFrame("torso_lift_link");
     group_arm_torso.setPoseTarget(goal_pose);
